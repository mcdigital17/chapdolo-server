export default async function handler(req, res) {
  ...
  try {
    ...
    if (getData.result) {
      ...
      try { const parsed = JSON.parse(rawResult || '{}'); if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) users = parsed; } catch (e) { users = {}; }
    }

    if (users[newUser]) return res.status(200).json({ success: false, message: 'Cet identifiant existe déjà' });
