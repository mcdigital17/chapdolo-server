...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (url.includes('.m3u8') && !contentType.includes('mpegurl')) contentType = 'application/vnd.apple.mpegurl';
                ...
                if (trimmed.startsWith('#')) {
                    if (trimmed.includes('URI="')) {
                        return trimmed.replace(/URI="([^"]+)"/g, (match, p1) => {
                            let finalUrl = p1.startsWith('http') ? p1 : new URL(p1, url).href;
                            return `URI="/api/proxy-stream?url=${encodeURIComponent(finalUrl)}&referer=${encodeURIComponent(ref)}"`;
                        });
                    }
                    return line;
                }
                let finalUrl = trimmed.startsWith('http') ? trimmed : new URL(trimmed, url).href;
